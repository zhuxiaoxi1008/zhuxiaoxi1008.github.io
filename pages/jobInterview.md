...
讲讲你做的最有挑战性的前端项目？难点怎么解决的？回答框架（STAR法）：
Situation：背景（什么系统、什么痛点）
Task：你的职责
Action：用了什么技术、怎么优化的、踩了什么坑
Result：数据提升 / 问题解决结果加分点：提到权限控制、XSS防护、性能监控、与安全团队协作的经历。
Vue2 和 Vue3 最大的区别？响应式原理区别？简答：
Vue2：Object.defineProperty，深度遍历、无法侦测新增/删除属性、数组需特殊处理
Vue3：Proxy + Reflect，直接代理整个对象，支持新增/删除、Map/Set/WeakMap，更高效
额外：Composition API、更小的打包体积、Teleport、Fragment、Suspense
加分点：说说 ref / reactive 区别、effectScope、<script setup> 语法糖。
Vue 组件间通信有哪些方式？（至少说6种）常见回答：
props / $emit
v-model / .sync（Vue2） / defineModel（Vue3）
provide / inject
全局事件总线（mitt / tiny-emitter）
...
$attrs / $listeners（Vue2） → attrs + emit（Vue3）
父子通过 ref 操作 / $parent / $children / $root
XSS 是什么？怎么分类？前端如何防御？分类：反射型、存储型、DOM-based
防御（前端能做的）：
输入：白名单校验、正则过滤
输出：对用户内容做转义（textContent / innerText > innerHTML）
富文本：使用成熟库 + 过滤（DOMPurify / js-xss）
...
避免 eval / new Function / innerHTML
加分点：提 DOM-based XSS 示例（location.hash / document.write）。
CSRF 是什么？怎么防御？简答：跨站请求伪造，利用用户登录态发送恶意请求。
防御：
SameSite=Strict/Lax Cookie
token（form 隐藏域 / header）
双重验证（密码/短信）
...
同源策略是什么？跨域有哪些解决方案？同源：协议+域名+端口三相同。
解决方案（按常用排序）：
CORS（后端 Access-Control-Allow-*）
jsonp（只限 get）
postMessage（iframe / window.open）
...
document.domain（主域相同子域）
WebSocket / SSE（不严格受同源限制）
浏览器从输入 URL 到页面渲染发生了什么？（经典全链路）简要版：
DNS解析 → TCP三次握手 → TLS握手（HTTPS） → 发送HTTP请求 → 服务端响应 → 浏览器接收 → 构建DOM/CSSOM → Render Tree → Layout → Paint → Composite
加分点：提到强缓存/协商缓存、304、preconnect/dns-prefetch、关键渲染路径优化。

二面（中高级 + 场景 + 安全深入）高频题

手写一个防抖 / 节流函数（至少两种实现）JavaScript// 防抖（最后一次执行）
...
你们项目是怎么做权限管理的？（RBAC / 前后端分离）前端：路由守卫 + 动态路由 + 按钮级权限（v-permission 指令）
后端：接口鉴权 + 数据权限
存储：用户信息 + 角色 + 权限码 → Pinia / localStorage
加分点：提到细粒度权限、权限缓存失效机制、越权校验。
前端怎么防止敏感信息泄露？（身份证、手机号等）
...
你对网络安全行业有什么了解？为什么想来绿盟？可说：国内头部网络安全厂商，主营态势感知、Web防火墙、等保测评等；
想来是因为对安全有兴趣、前端安全是Web安全重要一环、公司技术氛围好、能接触更多安全场景等。

准备建议（针对绿盟）：

Vue3 + TS + Pinia + Vite 项目最吃香
至少准备2个能讲深的项目（涉及权限/安全/中后台/可视化更好）
浏览器、网络、安全三块多背几道
能说出“我在XX项目中遇到XX安全问题，是这么处理的”会大幅加分